    ...
    if name == "internvl":
        ...
        for blk in runner.model.vision_model.encoder.layers:
            ...
            if v_attn is not None:
                ...
                hooks.append(v_attn.register_forward_hook(_internvl35_vision_attn_hook, with_kwargs=True))
            ...
            for blk in v_layers:
                ...
                v_attn = getattr(blk, "self_attn", None) or getattr(blk, "attn", None) or getattr(blk, "attention", None)
                if v_attn is None:
                    continue
                ...
                hooks.append(v_attn.register_forward_hook(_gemma3_vision_attn_hook, with_kwargs=True))

    else:
     
        #visual = getattr(getattr(runner.model, "model", None), "visual", None)
        blocks = runner.model.model.visual.blocks #getattr(visual, "blocks", None) if visual is not None else None
        if not blocks:
