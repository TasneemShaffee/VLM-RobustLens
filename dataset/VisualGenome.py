...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to unprocessed VisualGenome JSON",
    )
    parser.add_argument(
        "--output_json",
        type=Path,
        required=True,
        help="Path to save preprocessed VisualGenome JSON",
    )
...
def preprocess_json(json_path, output_path):
    ...
    for qa in flattened_json:
        rephrasings = gram_var_and_syn_rep(qa["question"], count=3) + back_translate(
            qa["question"], count=3
        )
...
def load_vg_vqa_rephrasings(json_path, images_dir):
    ...
    groups = defaultdict(
        lambda: {"questions": [], "answers": None, "image_id": None, "image": None}
    )
    for row in rows:
        g = groups[row["group_id"]]
        g["questions"].append(row["question"])  # type: ignore
