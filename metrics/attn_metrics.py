...
def _prepare_for_metric(A, B):
    ...
    if A.ndim in (3, 4):
        A = _avg_heads_if_needed(A)
    if B.ndim in (3, 4):
        B = _avg_heads_if_needed(B)
...
def _as_numpy_2d(x) -> np.ndarray:
    ...
    if x.ndim == 1:
        ...
        if s * s == n:
            ...
            raise ValueError(
                f"Attention map is 1D of len {n} and cannot be reshaped to square."
            )
...
def pearson_sim(A, B):
    ...
    if np.std(a) < EPS or np.std(b) < EPS:
        return 0.0
    return float(pearsonr(a, b)[0])  # type: ignore


def spearman_sim(A, B):
    a, b = _flatten(A), _flatten(B)
    return float(spearmanr(a, b).correlation)  # type: ignore
...
def token_alignment_consistency(
    text_emb_orig, text_emb_para, attn_orig, attn_para, sim="cosine"
):
    """
    ...
    if np.std(w1n) > EPS and np.std(w2n) > EPS:
        wp = float(pearsonr(w1n, w2n)[0])  # type: ignore
        ws = float(spearmanr(w1n, w2n).correlation)  # type: ignore
